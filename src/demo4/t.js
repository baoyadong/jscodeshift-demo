...
export default function transformer(file, api) {
  ...
  const updatedAnything = root.find(j.VariableDeclaration)
    .filter(desc => desc.value.kind === 'var')
    .filter(declaration => {
    ...
    .forEach(path => {
      const forLoopWithoutInit = isForLoopDeclarationWithoutInit(path);
      if(path.value.declarations.some(declarator => {
        return (!declarator.init && !forLoopWithoutInit) || isMutated(path, declarator)
      })) {
        path.value.kind = 'let';
      } else {
